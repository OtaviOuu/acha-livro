...
class RegisterTermView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if Termo.objects.filter(name=termo, user=user_associated).exists():
            return render(request, 'tracker/index.html', {'message': f'Termo "{termo}" já registrado!'})

        new_termo = Termo(name=termo, user=user_associated)
        new_termo.save()

        return render(request, 'tracker/index.html', {'message': f'Termo "{termo}" registrado com sucesso!'})
